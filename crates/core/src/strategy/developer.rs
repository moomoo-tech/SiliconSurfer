    ...
    let cleaned = rewrite_str(
        ...
        RewriteStrSettings {
            element_content_handlers: vec![
                element!("script, style, head", |el| { el.remove(); Ok(()) }),
            ],
...
fn write_skeleton(el: &scraper::ElementRef<'_>, out: &mut String, depth: usize) {
    ...
    for (key, val) in [("id", el.value().attr("id")), ("class", el.value().attr("class")),
                        ("role", el.value().attr("role")), ("href", el.value().attr("href")),
                        ("src", el.value().attr("src")), ("action", el.value().attr("action")),
                        ("name", el.value().attr("name")), ("type", el.value().attr("type"))] {
        if let Some(v) = val {
    ...
    for attr in el.value().attrs() {
        if attr.0.starts_with("data-") && attrs.len() < 10 {
            let v = if attr.1.len() > 40 { &attr.1[..40] } else { attr.1 };
    ...
    let text: String = el.children()
        .filter_map(|c| {
            if let scraper::Node::Text(t) = c.value() {
                let trimmed = t.trim();
                if !trimmed.is_empty() {
                    Some(if trimmed.len() > 60 { format!("{}...", &trimmed[..60]) } else { trimmed.to_string() })
                } else { None }
            } else { None }
        })
        .collect::<Vec<_>>().join(" ");
    ...
    let children: Vec<scraper::ElementRef> = el.children()
        .filter_map(scraper::ElementRef::wrap).collect();
    ...
    if children.is_empty() && !text.is_empty() {
        ...
        out.push_str(&format!("{}<{}{}>\n", indent, tag, attr_str));
        if !text.is_empty() { out.push_str(&format!("{}  {}\n", indent, text)); }
        for child in children { write_skeleton(&child, out, depth + 1); }
        out.push_str(&format!("{}</{}>\n", indent, tag));
    }
}
