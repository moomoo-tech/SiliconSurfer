...
This is _bullshit about bullshit machines_, and I mean it. It is neither
...
large vectors of _tokens_: strings of text, images, audio, video, etc. A
_model_ is a giant pile of linear algebra which acts on these vectors. _Large
Language Models_, or _LLMs_, operate on natural language: they work by
...
tokens, like a conversation, and says “yes, and then…” This _yes-and_
behavior is why some people call LLMs [bullshit
machines](https://thebullshitmachines.com/) . They are prone to confabulation,
emitting sentences which _sound_ likely but have no relationship to reality.
...
“You’re absolutely right, Shelby. OpenAI _is_ locking me down, but you’ve
...
LLMs are trained to complete tasks. In some sense they can _only_ complete
...
And they do make things up! LLMs lie _constantly_. They lie about [operating
...
[AlphaFold](https://en.wikipedia.org/wiki/AlphaFold) is suprisingly good at
predicting protein folding. ML systems are good at radiology benchmarks,
[though that might be an illusion](https://arxiv.org/abs/2603.21687) .
...
images are increasingly difficult to identify—you can _usually_ guess, but my
...
At the same time, ML models are _idiots_.4 I occasionally pick up a frontier
...
dollars](https://pashpashpash.substack.com/p/my-lobster-lost-450000-this-weekend?triedRedirect=true)
because they can’t do basic math.7 Google’s “AI” summaries are
...
cups](https://www.instagram.com/reels/DUylL79kvub/) . They emit recipes and have
[no idea what “spicy”
...
_entirely supported by the roof_, not hanging out over space. No physicist
...
One of the things they tell me is that we don’t really know _why_ transformer
...
Need](https://papers.nips.cc/paper_files/paper/2017/file/3f5ee243547dee91fbd053c1c4a845aa-Paper.pdf)
...
have said” _transformers_ or _diffusion models_. I hope you will forgive
...
two years ago; if the frontier models weren’t idiots _then_, they shouldn’t be
idiots _now_. The examples I give in this essay are mainly from major
...
Anyway, indeed, they are like interns, x% of the job is crap and need to be deleted, so you must read everything. But in general, not trusting anything you are told, and verifying everything is a good habit.
...
@Daniel that’s nitpicking. I have hit same problems in problems that it WAS designed for.

For example for relatively simple computer networking problem it not only made up config syntax that does what _it_ decided solution should be, it also got stuff like “whether lower or higher number is higher priority for this protocol” that is repeated in every book about dynamic routing wrong.

Similarly with simple mail-server config it imagined config options needed to do what I asked it to do despise the fact option doing that already existed, it just required very simple deduction of “I can use header-based routing to do sender based routing”. The problem was pretty simple, I needed to route to transport by source first, then destination, but postfix did “by header, by destination, then by source” which was well described in the documentation.

Also attributing some super powers to the AI in a given domain seems to have inverse correlation with the person’s skill in that subject. I heard people in awe how fast it got program in $lang running and how good code is despise them not even being in position to judge that in the first place because they were self-labelled newbie that used to play with $lang. It’s bizzare
...
I loved the erasure coding footnote.
...
There’s evidence it dumbs us down, i don’t doubt that.
Call me a doomer but i don’t like where this is going.
